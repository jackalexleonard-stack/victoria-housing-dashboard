...
test('two secondary finding cards render the next hero picks, excluding the lead', async () => {
  ...
  // rather than first-class, but this shim (.map(e => e.key)) only
  // consumes the key, so the pool of keys is unchanged from pre-migration.
...
test('no hero_lead: the conveyor leads with the first hero pick, no crash', () => {
  ...
  // badged at its own geo rather than first-class, but the shim only
  // consumes the key.
